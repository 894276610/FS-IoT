...
//     arma::rowvec new_row = {7, 8};  
//     new_row = mlpack::join_rows(new_row,  arma::rowvec{97});
//     // 在最后插入新行（行索引=当前行数）

//     dataset.insert_rows(dataset.n_rows, new_row);
//     dataset.insert_rows(dataset.n_rows, new_rows);
//     dataset.resize(1,4);
//     dataset.fill(0);
...
int main()
{
  ...
  data::Split(dataset, labels, trainSet, testSet, trainLabels, testLabels, 0.3 
    ...
    std::cout << "label n_rows:" <<   trainLabels.n_rows << ", n_cols:" << trainLabels.n_cols;
    std::cout << "data n_rows:" <<   trainSet.n_rows << ", n_cols:" << trainSet.n_cols;
